...
def transcribe(event, context):
	...
	secretResponse = secrets.get_secret_value(SecretId=WATSON_TRANSCRIBE_CREDENTIALS)
	...
	if callbackResponse.status_code == 200 or callbackResponse.status_code == 201:
		for record in event['Records']:
			...
			for output in message['outputs']:
				...
				response = startTranscriptionJob(audioFile, episode, credentials, WATSON_CALLBACK_URL)
				logger.info('Started Recognition job with response code: ' + str(response.status_code) + ' and body ' + response.text)
	else:
		logger.info('Watson callback registration failed with response code: ' + str(callbackResponse.status_code) + ' and body ' + callbackResponse.text)
...
def download(event, context):
	...
	if status['event'] == 'recognitions.completed':
		logger.info('Fetching Watson transcription service credentials from AWS Secrets Manager.')
		# Load the watson transcription service credentials from the AWS Secrets Manager.
		secretResponse = secrets.get_secret_value(SecretId=WATSON_TRANSCRIBE_CREDENTIALS)
		credentials = json.loads(secretResponse['SecretString'])
		logger.info('Retrieved Watson transcription service credentials from AWS Secrets Manager.')

		logger.info('Fetching transcription results from Watson for job: ' + status['id'])
		url = TRANSCRIBE_BASE_URL + '/' + status['id']
		data = requests.get(url, auth=(credentials['username'], credentials['password']), stream=True)
		logger.info('Retreived transcription results from Watson for job: ' + status['id'])

		episode = status['user_token'].split('/')[0]
		offset = status['user_token'].split('/')[1]
		filename = status['id'] + '-' + episode + '-' + offset + '.json'

		logger.info('Uploading transcription results to S3 ' + OUTPUT_BUCKET + '/' + filename)
		bucket = s3.Bucket(OUTPUT_BUCKET)
		bucket.upload_fileobj(data.raw, filename)
		logger.info('Finished uploading transcription results to S3 ' + OUTPUT_BUCKET + '/' + filename)
		...
		logger.debug('Sent message to Watson download complete SNS ' + json.dumps(response))
	else:
		logger.info('Skipping processing of ' + status['event'] + ' event from Watson.')
...
def normalize(event, context):
	...
	for record in event['Records']:
		...
		if len(episode.splitWatsonTranscriptions) == len(episode.splits):
			...
			response = topic.publish(Message=json.dumps({'default': json.dumps(message)}), MessageStructure='json')
			logger.debug('Sent message to Watson normalization complete SNS ' + json.dumps(response))
