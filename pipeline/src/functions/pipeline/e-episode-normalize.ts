...
import { google as googleTranscribe, normalized } from '../../utils/transcribe'
...
const episodeNormalizeHandler = async (lambda: Lambda, job: Job, episode: EpisodeJob) => {
  ...
  let googleComplete = 0
  let transcriptionErrors = false
  const totalSegments = episode.segments.length

  try {
    googleComplete = await googleTranscribe.transcriptionsReadyToBeNormalized(episode)
    await job.log(`${googleComplete} of ${totalSegments} Google transcriptions are complete.`)
  } catch (error) {
    await job.logError('Error while checking if the transcriptions are ready.', error)
    transcriptionErrors = true
  }

  if (googleComplete === totalSegments) {
    await job.log(`Fetching the transcriptions for ${episode.podcastSlug} ${episode.slug}.`)
    const googleTranscriptions = await googleTranscribe.getEpisodeTranscriptions(episode)

    await job.log(`Zipping ${googleTranscriptions.length} segments into a single transcription.`)
    const googleTranscription = normalized.appendAllTranscriptions(googleTranscriptions)
    await aws.s3.putJsonFile(episode.bucket, episode.transcriptions.google, googleTranscription)
    await aws.s3.putJsonFile(episode.bucket, episode.transcriptions.final, googleTranscription)

    const statements = normalized.getSentences(googleTranscription)
